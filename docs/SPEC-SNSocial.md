...
- Локали ru (по умолчанию) и en, все тексты — в `messages.yml`, цвета в четырёх записях
  (`&c`, `&#RRGGBB`, `&x&R&R&G&G&B&B`, MiniMessage), пустой ключ `prefix` убирает название
  плагина из всех строк разом.
...
- Главный поток не делает I/O во время игры: сеть и JDBC — всегда в фоне. Диск он трогает
  только на enable и по `/snsocial reload`: `config.yml` и `messages.yml`, по одному файлу,
  один раз за перезагрузку — это осознанное исключение, тексты нужны синхронно всему, что
  собирается следом.
- Каждый публичный текст — из `messages.yml`, UTF-8, ru по умолчанию.
...
| Главный поток | ни сети, ни JDBC; диск — только на enable и `/snsocial reload` | Ревью: I/O только на своих потоках; в игровой поток — dispatch команд и GUI |
...
| Локали | ru из коробки; `language: en` выбирает язык, на котором создаётся `messages.yml`, дальше язык задаёт сам файл | встроенные messages-ru.yml / messages-en.yml, поверх них правится `plugins/SNSocial/messages.yml`; файл помечен ключом `language`, расхождение с флагом — предупреждение в консоли (фикстура); UTF-8 везде |
| Цвета в текстах | `&c`, `&#RRGGBB`, `&x&R&R&G&G&B&B`, MiniMessage — вперемешку | `core/Colors.java`, фикстуры `colours()` в самотесте |
...
- **Messages**: паритет ru/en по всем ключам; `bot.*` без разметки; `{prefix}` не доживает до
  вывода; пустой `prefix` (и `prefix: ''`, и `prefix:` без значения) убирает название плагина
  из всех строк; правка поверх встроенного набора; удвоенный апостроф; перенос правок из
  `messages-ru.txt` 26.8.1 с сохранением комментариев шаблона; второй `messages-*.txt` назван
  в логе, а не перенесён молча; расхождение `general.language` и языка `messages.yml` даёт
  строку в консоль, совпадение — не даёт.
- **Colors**: `&c`, `§a`, `&#RRGGBB`, `&x&R&R&G&G&B&B`, украшения и `&r`; одиночный `&` —
  текст; легаси-сброс жирного там, где нет тегов, и его отсутствие там, где теги есть;
  `&` внутри аргумента известного тега остаётся частью аргумента — `?a=1&b=2` в
  `<click:open_url:...>` не превращается в `<aqua>`, — а в `<5 и &a>` тега нет, и код цвета
  там по-прежнему код цвета;
  экранирование MiniMessage понимают оба приёмника одинаково: в чат уходит `\<red>`, боту —
  текст `<red>`, а обратный слэш перед новым тегом удваивается, иначе тег съело бы;
  `strip` для ботов режет коды и известные теги, но не `<игрок>` и не кусок предложения
  между обычным `<` и далёким `>`; согласие обоих путей проверяется 200 000 случайных строк
  на пяти фиксированных сидах, алфавит включает обратный слэш и оба вида кавычек.
